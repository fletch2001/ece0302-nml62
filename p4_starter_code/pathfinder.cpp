...
int main(int argc, char *argv[]) {
    ...
    if (argv[1] == 0)
        exit_handler("missing input file argument");
    else if (argv[2] == 0)
        exit_handler("missing output file argument");
...
void search(metadata &m, const coord &c) {
    ...
    while (true) {
        ...
        if (explored[current.i][current.j] == 1)
            continue;  // skip to next coordinate in queue
        else {
