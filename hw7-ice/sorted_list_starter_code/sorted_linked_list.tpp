...
{
  Node<T> * thisPtr = getNodeAt(0); // get head ptr
  Node<T> * nextPtr = nullptr; // next pointer to get

  for(unsigned i = 0; i < list.getLength(); i++) {
    //nextPtr->getNext(); 
    nextPtr = thisPtr->getNext(); // get pointer to next node from current node
    if(thisPtr->getItem() >= nextPtr->getItem()) { //perform insertion
